...
export function makeSemanticColorTokens(
  ...
  // Value is an object containing 1-3 keys of dark, light, contrastText
  ...
  TOKENS.forEach((item: Frame) => {
    ...
    if (item.name.startsWith('$')) {
      ...
      if (isMuiToken(NAME)) {
        ...
        // Add a record to the dictionary to store light, dark, contrastText
        // if it doesn't exist...
...
function parseSemanticColorToken(
  item: Frame,
  outputFormatColors: OutputFormatColors,
  colorPrimitives: any,
  camelizeTokenNames?: boolean
